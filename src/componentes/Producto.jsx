...
const s3_p = 'https://tratego246b0be5c8cb404cb931ed626cc6cc1e161530-dev.s3.sa-east-1.amazonaws.com/public/producto';
...
class Producto extends React.Component {
    ...
    cargarProducto = async () => {
        if(this.props.params!==undefined){
            const fil = _.where(this.state.productos, {id_inventario: parseInt(this.props.params.id)});
            await this.setState({producto: fil[0]})
        }else{
            await this.setState({producto: []});
        }
    }

    agregarCarrito = async (prod) => {
        const { cookies } = this.props;
        var con = this.state.carrito;

        var num = con.filter((obj) =>{
            var num = 0
            if(obj.id_inventario === prod.id_inventario){
                num+=1
            }
            return num
        })
                ...
                <div className="seleccion_p">
                    <div className="img_produ_g2">
                        <Carousel itemsToScroll={1} itemsToShow={1}>
                            <img src={`${s3_p}/${this.state.producto.archivo}`} alt=""/>
                            <img src={`${s3_p}/${this.state.producto.archivo}`} alt=""/>
                            <img src={`${s3_p}/${this.state.producto.archivo}`} alt=""/>
                            <img src={`${s3_p}/${this.state.producto.archivo}`} alt=""/>
                        </Carousel>
                    ...
                    <div className="cuadro img_produ_g">
                        {this.props.cookies.get('persona').tipo === "club" && 
                            <div className="cuadro_producto2">
                                <div>
                                <label className="producto_nombre">{this.state.producto.nombre}</label>
                                <br/>
                                <div className="prec_cant">
                                    <div>
                                        <label className="producto_precio">USD $ {this.state.producto.valor_venta}</label>
                                    </div>
                                    <div>
                                ...
                                <button className="agregar_carrito" style={{background: this.props.cookies.get('persona').color2}} onClick={()=>this.agregarCarrito(this.state.producto)}>Agregar al carrito</button>
                                </div>
                            </div>
                        }
                        ...
                        <div className="cuadro_producto">
                            ...
                            <br/>
                            <label className="producto_descripcion">{this.state.producto.descripcion}</label>
                        </div>
                    </div>
...
export default withCookies(withRouter(Producto));